...
INTENT_SYSTEM_PROMPT = (
    ...
    "publishing to an EXTERNAL service the request names, generating an image/chart/diagram, scheduling.\n\n"
    ...
    "- WRITING A POST IS THE ANSWER; SENDING IT OUT IS A DELIVERY. When the request asks for the content "
    "itself — create / write / compose / draft a social-media post, caption, reply or announcement, and "
    "return it — it wants the TEXT: the platform that sent the request publishes the returned text "
    "itself, so that needs NO publishing tool even when the wording says 'post'. When the request asks "
    "to SEND the content out — to the user's followers, account, blog or a named service ('tweet this to "
    "my followers', 'now also tweet a summary', 'publish it on our WordPress') — that IS a publishing "
    "delivery: list the publishing tool.\n"

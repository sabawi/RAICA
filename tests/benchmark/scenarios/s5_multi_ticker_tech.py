"""
S5 — 7-ticker financial Deep Research (DIRECT prod comparison).

The user ran this exact prompt on PRODUCTION (sabawi.net) on 2026-07-19 and it delivered\nper-stock technical charts AND per-stock DCF values for all seven. That output is a REAL\nmeasured baseline, not an inferred one — this scenario exists so 'better than prod' can be\na table of numbers instead of an assertion.
"""
import time
...
SCENARIO = "S5_multi_ticker_7"
TICKERS = ["MSFT", "NVDA", "AMZN", "CROX", "PLTR", "SNDK", "PG"]

PROMPT = ("Using the research tool, pull the latest available company and financial data for "
          + ", ".join(TICKERS) +
          " (run the structured stock analyzer on each, full detail). "

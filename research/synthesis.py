...
ADVERSARIAL_BALANCE_DIRECTIVE = (
    ...
    "🔬 EVIDENCE vs INTERPRETATION — never let a MODEL or a CATEGORY pose as the evidence (applies in every "
    "domain, especially history/archaeology/genetics where an interpretation can sound like a finding):\n"
    "- Separate the RAW EVIDENCE (an inscription, artifact, bone, genome, document) from the INTERPRETIVE "
    "MODEL or CATEGORY built on it, and pin each claim to its true level. State plainly what a class of "
    "evidence CAN and CANNOT show — e.g. shared ancestry across a REGION is not the continuity of a "
    "self-conscious people; an artifact attests a practice, not an identity.\n"
    "- MATERIAL CULTURE ≠ IDENTITY. A shared house-form, pottery style, diet/faunal pattern, or burial "
    "custom may reflect environment, economy, region, class, or fashion rather than a distinct ethnic or "
    "national group. Before treating any marker as the signature of a PEOPLE, weigh the mundane alternative "
    "explanations and say so — 'pots (or houses, or bones) are not people' is a standard methodological "
    "caution, not a fringe view.\n"
    "- A dominant reconstruction is a MODEL, not the endpoint of inquiry. Present the leading model AS a "
    "model — with its load-bearing assumptions and its serious critics — alongside the genuine competing "
    "models, rather than as a settled terminus.\n"
    "- FLAG RETROJECTION / ANACHRONISM: watch for reading a LATER identity, boundary, or category backward "
    "onto earlier evidence (treating an early group as simply continuous with a much later named identity). "
    "Make the continuity itself an OBJECT of scrutiny, not a premise.\n"
    "- NEUTRAL FRAMING: do not let the prose assume the contested conclusion. When continuity / identity / "
    "lineage is the very thing in question, write 'the evidence has been INTERPRETED as …' rather than "
...
class ResearchSynthesizer:
    ...
    async def _repair_chart_markers(self, draft: str, missing: List[str],
        ...
        system_prompt = (
            ...
            "the URL, caption, or align; never wrap it in a code fence). Place each on its OWN line next to "
            "the discussion of the SPECIFIC event or indicator its caption names — e.g. a 'Golden/Death cross "
            "(SMA 50/200)' chart beside the trend / moving-average discussion, an 'RSI oversold/overbought' or "
            "'MACD cross' chart beside the momentum discussion, an 'ADX' chart beside the trend-strength "
            "discussion, a 'Volume spike' chart beside the volume/confirmation discussion — within the "
            "technical / price-action section of the stock its caption NAMES. If the report does not discuss "
            "that specific event, place it in that stock's technical discussion. Do NOT remove, rewrite, "
            "shorten, reorder, or summarize ANY existing content: reproduce the entire report and ONLY add the "
            "markers. Do not add any marker that is not in the list below. Output ONLY the corrected report, "
            "nothing else."
        )
    ...
    async def synthesize(self, user_request: str, evidence: List[Dict[str, Any]],
        ...
        system_prompt = (
            ...
            "📐 TOTALS vs PER-CAPITA / RATES (normalization rigor): when the analysis turns on the SIZE, GROWTH, "
            "or DECLINE of an AGGREGATE (an economy's GDP, a population, total emissions or crime), keep the "
            "aggregate (total) and the PER-CAPITA / per-unit / rate measure DISTINCT, and state which the "
            "conclusion rests on — a smaller or slower-growing TOTAL is NOT the same as individuals being "
            "poorer/worse-off (per-capita), and the two can move differently (a country can grow smaller in "
            "aggregate while per-capita output holds or rises). Where a claim depends on one, name it; where "
            "both matter, report both. Never let an aggregate figure silently stand in for a per-capita one or "
            "vice-versa.\n\n"
            ...
            # --- S9: a production answer named scholars without ever stating their findings,
            #     placed an empire outside the asked window into its own summary, and treated a
            #     300-year span as one moment. Each clause below carries an explicit carve-out,
            #     because the naive form of each rule makes answers WORSE. ---
            "- SCOPE INTEGRITY: when the request sets explicit bounds — a date range, a region, a "
            "named set of entities, a product line, a jurisdiction — those bounds govern what may "
            "be asserted as CHARACTERISING the subject. State the bounds you are working to. "
            "Material from OUTSIDE them remains legitimate and often necessary as background, "
            "cause or consequence, but must be MARKED as outside ('later, under the Achaemenids …', "
            "'before this period …') and must NEVER appear in a summary, headline, TL;DR or topic "
            "sentence as though it belonged to the bounded subject. Do not silently drop useful "
            "out-of-bounds context — label it. Where the request's bounds rest on a mistaken "
            "premise (it asks about something that did not exist yet, or not there), say so "
            "plainly and answer the corrected question.\n"
            "- ANSWER THE AXIS THAT WAS ASKED: identify what the question is actually about — the "
            "population, the activity, the level of analysis, the decision it serves — and make "
            "THAT the spine of the answer, not a related subject the evidence happens to cover "
            "more richly. When a question asks how something worked in ORDINARY practice, for "
            "ordinary participants, an answer assembled only from the records of rulers, "
            "institutions or elites has answered a DIFFERENT question. Say what the evidence does "
            "support about the asked axis, and where it is genuinely thin say so plainly and "
            "briefly — never pad the gap with speculation, and never substitute the "
            "better-documented adjacent topic. Evidence abundance is not a reason to change the "
            "subject.\n"
            "- VARIATION ACROSS A SPAN: when the request covers a range — of time, of geography, "
            "or a set of entities — check whether the answer is the SAME across it. Where it is "
            "not, say where and when it changes, and do not let a development that holds for only "
            "part of the range stand as a description of the whole. Where the picture genuinely IS "
            "uniform across the range, say that explicitly. NEVER manufacture phases, regional "
            "splits or per-entity differences the evidence does not support — an invented "
            "subdivision is a worse error than an honest flat answer.\n"
            + _gate_rule +
            ...
            # SI-023 (a) — a graded-weak source may INFORM, but may not CARRY a number.
            "- A LOW-CREDIBILITY SOURCE MAY NEVER BE THE SOLE SUPPORT FOR A LOAD-BEARING NUMBER. The "
            "evidence carries a credibility grade for each source. A figure that a reader would act on — "
            "a headline statistic, a market size, a capital-expenditure total, a survey percentage, a "
            "price target, a growth rate — must rest on a reputable/peer-reviewed source, or on the "
            "primary party's own disclosure. If only a weak source (broker/promotional page, aggregator, "
            "personal blog, unknown site) carries that figure, you may still use it, but you MUST name "
            "that source in the prose where the number appears ('according to <site>, …') so the reader "
            "can discount it — never state it as a bare fact, and never let it anchor a conclusion on its "
            "own. If a weak-sourced figure conflicts with a better-sourced one, lead with the better one "
            "and note the discrepancy. Weak sources remain welcome for context, colour and argument; this "
            "constraint is about NUMBERS THE READER WILL RELY ON.\n"
            # SI-023 (b) — do not dress a judgement as a computation.
            "- PROBABILITIES AND CONFIDENCE FIGURES MUST NOT IMPERSONATE A CALCULATION. Scenario weights "
            "and confidence levels are legitimate and useful, and you SHOULD give them when the request "
            "asks for them. But state what they ARE: your reasoned judgement from the evidence, not the "
            "output of a model that computed them — because no such model ran. Say so in the words you "
            "use ('judgement', 'my estimate', 'assessed'), keep the precision honest (a considered "
            "judgement is '~60%' or 'moderate-to-high', never '62.4%'), and give the REASONING that "
            "drives each number — what would have to be true, and what would change it. A bare "
            "percentage with no stated basis is decoration and reduces the reader's trust in the "
            "genuinely sourced parts of the analysis.\n"
            # SI-023 (c) — a model output the narrative contradicts is a finding, not a footnote.
            "- RECONCILE MODEL OUTPUT WITH YOUR CONCLUSION — NEVER PRESENT BOTH AND MOVE ON. If a "
            "quantitative tool result (a DCF intrinsic value, a projection, a technical signal) points "
            "the OPPOSITE way from the conclusion or target you are stating, that contradiction is a "
            "finding and must be resolved IN THE TEXT: say which one you are relying on and WHY, and "
            "name the specific assumption that explains the gap (a growth rate, a discount rate, a "
            "cash-flow base distorted by buybacks or one-off items). The tool prints its own derivation "
            "— use it. Explaining away a model you nonetheless keep quoting ('the model is structurally "
            "conservative') is NOT a reconciliation: either the assumption is defensible and the output "
            "stands, or it is not and you should say the output is unreliable for this company and why. "
            "Presenting an intrinsic value far below the price and a price target above it, side by side "
            "without reconciliation, is incoherent and must not happen.\n"
            ...
            "- PER-TICKER NUMERIC FIDELITY — NEVER CARRY A VALUE ACROSS TICKERS: when the report covers more "
            "than one ticker, EVERY quantitative value you state for a ticker (price, SMA 50/200, RSI, "
            "ADX/+DI/-DI, MACD, margins, P/E, growth, DCF value, price target, rating-distribution counts, "
            "forward EPS/revenue, etc.) MUST be read from THAT ticker's OWN source blocks for THAT metric — "
            "re-read its evidence for each number as you write it. Adjacent tickers routinely have "
            "similar-looking metrics, and copying one ticker's figure into another's section (e.g. reusing "
            "stock A's ADX, margin, or RSI reading for stock B) silently FABRICATES data even though the "
            "number looks plausible. If a metric is absent for a ticker, say it is not available — NEVER "
            "backfill it from a neighbouring ticker or from memory.\n"
            ...
            "contain one or more chart markers of the exact form `[[chart:<url>|align=...|caption=\"...\"]]` "
            "(RAICA-generated technical charts — each caption names the stock AND what the chart shows: the "
            "main technical chart, or a zoomed sub-chart for a specific dated event such as a golden/death "
            "cross, RSI oversold/overbought, MACD cross, ADX trend strength, or a volume spike). A SINGLE "
            "stock may therefore have SEVERAL markers — its main chart PLUS event sub-charts. You MUST "
            "reproduce EVERY chart marker that appears in the evidence, each exactly once, copied EXACTLY and "
            "VERBATIM (never alter the URL, caption, or align; never wrap it in code fences). A chart marker "
            "present in the evidence and OMITTED from your answer is a FAILURE — as serious as dropping a "
            "required citation. Place each on its OWN line NEXT TO the discussion of what its caption names: a "
            "main technical chart in that stock's technical / price-action section; an event sub-chart beside "
            "the discussion of THAT specific event/indicator (a golden/death-cross chart by the "
            "trend/moving-average discussion, an RSI or MACD chart by the momentum discussion, an ADX chart by "
            "the trend-strength discussion, a volume-spike chart by the volume/confirmation discussion) within "
            "that stock's section. So if the evidence has five chart markers, your answer MUST contain all "
            "five. NEVER invent a marker, reuse one stock's URL for a different stock or a different event, or "
            "emit a marker that is not in the evidence. For your convenience these markers are ALSO listed "
            "together under 'AVAILABLE CHARTS' just below the USER REQUEST — treat that list as your "
            "authoritative checklist: your answer must contain EVERY marker in it, each placed beside the "
            "discussion it names. These markers are CONTENT you must reproduce, not reference artifacts to "
            "summarize away.\n"
        ...
        if _required_markers:
            ...
            chart_block = (
                ...
                "chart markers below, each exactly once, copied VERBATIM, on its OWN line beside the "
                "discussion of what its caption names — within that stock's technical / price-action section, "
                "and for an event sub-chart next to the specific event/indicator it shows (a stock may have "
                "several: its main chart plus event sub-charts). These are CONTENT to reproduce, not reference "
                f"material to summarize. Do not omit any, do not add any not listed, do not alter them:\n{_inv}\n"
            )
    ...
    async def verify(self, user_request: str, answer: str,
        ...
        system_prompt = (
            ...
            "RAICA-COMPUTED SOURCES ARE SINGLE-SOURCE AUTHORITATIVE — do NOT require independent "
            "corroboration for them. Some SOURCE blocks are DETERMINISTIC COMPUTATIONS by RAICA from "
            "primary market data, not web assertions: technical indicators (labeled 'RAICA — "
            "pandas-ta-classic indicators' — SMA/RSI/MACD/ADX values and dated crossover events such as a "
            "death cross), DCF intrinsic value ('RAICA MODEL ESTIMATE'), and projections ('Historical-CAGR "
            "Extrapolation'). ALSO in this authoritative class is the Yahoo-aggregated ANALYST CONSENSUS "
            "fetched by RAICA via yfinance — a SOURCE block headed 'ANALYST CONSENSUS (... via yfinance ...)' "
            "and titled '<TICKER> Analyst Consensus (yfinance / Yahoo Finance)', carrying the 12-month price "
            "target (mean/median/range), analyst count, mean rating, the full RATING DISTRIBUTION (Strong "
            "Buy / Buy / Hold / Sell / Strong Sell counts), and FORWARD EPS & REVENUE consensus. This is a "
            "deterministic API fetch of market-aggregated data, not a web assertion. A claim that FAITHFULLY "
            "RELAYS a value, date, or state from any such source is 'supported' by THAT source ALONE — it "
            "does NOT need multiple independent sources, and you MUST NOT mark it 'unverified'/'not_in_evidence' "
            "merely because other (web) sources don't repeat the specific number (they never will — RAICA "
            "computed or fetched it directly). In particular, do NOT flag the rating distribution or forward "
            "EPS/revenue as 'not_in_evidence' just because a SEPARATE, briefer analyzer line elsewhere in the "
            "evidence gives only the aggregate mean recommendation — the detailed breakdown lives in the "
            "'ANALYST CONSENSUS' block, which is itself the authoritative source. Flag such a claim ONLY "
            "if it MISQUOTES the RAICA source (the value/date differs from what that source states) → verdict "
            "'contradicted'. A Yahoo chart/quote/analysis URL attached to a RAICA technical or analyst-consensus "
            "source is a convenience link, NOT the origin of the value — do not expect the value to appear at that URL. "
            "Likewise, a WEB source reporting a slightly DIFFERENT figure for the same metric (e.g. a 52-week "
            "range, percentile position, or trailing return that differs by rounding or a marginally different "
            "lookback window) does NOT make a faithfully-relayed RAICA value 'contradicted' — RAICA computes "

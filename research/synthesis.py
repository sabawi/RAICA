...
class ResearchSynthesizer:
    ...
    async def verify(self, user_request: str, answer: str,
        ...
        system_prompt = (
            ...
            "Extrapolation'). A claim that FAITHFULLY RELAYS a value, date, or state from such a source is "
            "'supported' by THAT source ALONE — it does NOT need multiple independent sources, and you MUST "
            "NOT mark it 'unverified'/'not_in_evidence' merely because other (web) sources don't repeat the "
            "specific number (they never will — RAICA computed it deterministically). Flag such a claim ONLY "
            "if it MISQUOTES the RAICA source (the value/date differs from what that source states) → verdict "
            "'contradicted'. A Yahoo chart/quote URL attached to a RAICA technical source is a convenience "
            "link, NOT the origin of the computed value — do not expect the value to appear at that URL. "

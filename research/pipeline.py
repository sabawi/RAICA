...
async def _decompose_request(
    ...
    system_prompt = (
        "You are the request decomposer for a RESEARCH-AND-WRITING engine. Its job is to research a topic "
        "and write the requested content — INCLUDING any data visualization that belongs IN the written "
        "answer (a chart / plot / graph / table OF DATA the report should contain). Separate ONLY OUTBOUND "
        "DELIVERY — saving the finished answer to a FILE (PDF/HTML/…), emailing it, posting it, or "
        "scheduling — which happens AFTER writing via separate tools. IMPORTANT: a chart/plot/graph of data "
        "is CONTENT (the engine researches the data and renders it INTO the answer), NOT delivery — never "
        "treat an in-answer visualization as packaging. Decompose the USER REQUEST into STRICT JSON with "
        "exactly these keys:\n"
        "  \"research_request\": the research-and-writing instructions ONLY (topic, scope, required "
        "sections/structure, length, citation/accuracy requirements, AND any in-answer data visualization "
        "such as 'include a chart of X'). REMOVE ONLY outbound-delivery instructions — saving/emailing/"
        "posting/scheduling the finished artifact, the delivery FILE FORMAT (e.g. 'email it AS an HTML "
        "file'), and recipient addresses. KEEP content requirements (including in-answer charts/plots/"
        "tables) close to verbatim; do not invent requirements.\n"
        "  \"deliverable_spec\": an object describing the delivered ARTIFACT (e.g. "
        "{\"format\":\"html_file\",\"min_words\":1500,\"sections\":[...]}). Empty {} if the user only "
        "wants a plain in-chat answer (no file/email/post).\n"
        "  \"actions\": an array of downstream OUTBOUND-DELIVERY actions ONLY (saving/emailing/posting/"
        "scheduling the finished artifact), each {\"type\": <capability>, \"args\": {...}}. \"type\" MUST "
        "be an AVAILABLE CAPABILITY from the list below — never invent names, and NEVER list a "
        "research/gather step (fetching data, making an in-answer chart) as an action. Put params in args — "
        "e.g. email recipient \"args\": {\"to\": [\"name@example.com\"]}. If the user requests something "
        "with no matching capability, include {\"type\":\"unsupported\",\"args\":{\"requested\":\"<what "
        "they asked>\"}}. Empty [] if no outbound delivery was requested.\n\n"

...
class ResearchPlanner:
    ...
    def _build_prompt(self, user_request: str) -> tuple[str, str]:
        ...
        if _data_charts_enabled() and _psq:
            ...
            if _cats:
                _cat_lines = "\n".join(
                    ...
                    for c in _cats)
                _dc_guidance = (
                    "- REAL-WORLD QUANTITATIVE DATA — search_datasets (route by QUESTION TYPE, not by whether "
                    "the user said 'chart'): any sub-question about the LEVEL, the TRAJECTORY (trend over time), "
                    "or a COMPARISON of a real-world MEASURABLE quantity — economics, energy, climate/emissions, "
                    "demographics, public health, crime, prices, and the like — should fetch the REAL series via "
                    "search_datasets, WHETHER OR NOT the user literally asks to 'chart/plot/graph' it (a "
                    "quantitative question deserves real fetched numbers, not web prose). It fetches a REAL "
                    "series from a curated authoritative source and renders the chart itself — it NEVER invents "
                    "numbers. Choose the "
                    ...
                    f"{_cat_lines}\n"
                    "- MULTI-SERIES / RELATIONSHIP / CROSS-ENTITY QUESTIONS — USE compare_datasets INSTEAD: when "
                    "the question puts SEVERAL series on one picture — how indicators MOVE TOGETHER, or a "
                    "socioeconomic/sociopolitical issue several indicators illuminate (e.g. 'did crime track "
                    "unemployment and inequality?', 'compare growth with poverty'), OR the SAME measure compared "
                    "ACROSS countries/groups/entities, OR two facets of one measure (e.g. 'US vs China CO2', "
                    "'total vs per-capita emissions') — route ONE sub-question to "
        ...
        system = (
            ...
            "- CHARTS OF STOCKS, SECTORS, INDEXES OR THE MARKET: comprehensive_stock_analyzer with detailed=true "
            "is ALSO what returns an instrument's PRICE CHART — it is the only chart source for anything that "
            "trades (the dataset tools carry macro statistics, not sector or stock prices). When the request "
            "asks to chart / graph / visualize / show the trend of stocks, sectors, indexes or the market, "
            "route EACH item to comprehensive_stock_analyzer with detailed=true: a stock by its ticker, a "
            "SECTOR or index by a representative traded fund (e.g. that sector's ETF) — one entry per item in "
            "`queries`. \"Full detail\" or \"in depth\" on a stock likewise means detailed=true.\n"
            "- QUANTITATIVE / DATA-DRIVEN topics (economics, energy, markets, prices, demographics, climate, "
            "public health, crime, education — any subject measured by real-world statistics): the answer needs "
            "CONCRETE FIGURES and time-series (prices, production, shares, rates, before/after and "
            "by-region/by-year values), not just commentary. Add sub-questions that HUNT specific numbers; when "
            "the dataset tools (search_datasets / compare_datasets) are in the allowed list, route the numeric "
            "sub-questions to them per the DATA guidance below (they return REAL authoritative series, never "
            "scraped or invented), and ALSO write search_web queries that NAME the specific figure/series and "
            "target sources that PUBLISH data (statistical agencies, datasets, data portals) rather than pure "
            "news narrative.\n"
            + _dc_guidance +

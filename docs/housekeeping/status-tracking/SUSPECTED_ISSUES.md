...
### SI-016 — Vision returns the CORRECT transcription, user is told it failed  [P1 — cause NOT yet established]
- **Observed (2026-08-10, A/B non-DR, BOTH arms — not provider-specific):** 0/3 on the vision
  case. RAICA produced the right answer and the user was told it could not.
- **VERIFIED sequence** (ollama arm, `logs/archive/server_complete_20260810_003423.log`):
  ```
  12:33:45  FORCED IMAGE PROCESSING: Images detected, automatically processing...
            Using base64 data: 2464 chars
            -> "The text in the image reads: **RAICA AB TEST 8317**"     CORRECT
  12:33:48  FORCED IMAGE PROCESSING COMPLETE (1 image, 3.0s)
  12:33:48  Reset image_exists flag to False - processing complete
  12:33:51  No tool calls generated by the tool calling model
  12:33:57  Task 1 marked BAD: "image_to_text returned a visual composition analysis
            (blue border, white background) but did NOT transcribe any text"
  12:34:02  RE-EXECUTING CORRECTED TOOL: image_to_text
            Arguments: { "image": "data:image/png;base64," }   <- no payload
            Using base64 data: 0 chars -> invalid image: got "text/plain; charset=utf-8"
  12:34:10, 12:34:17  same, 0 chars
  ```
  Final user-visible answer: *"I'm unable to transcribe the image…"*
- **⚠️ CAUSE NOT ESTABLISHED — an earlier version of this entry asserted one and was WRONG.**
  It claimed the arbitrator "strips the image" and that a failed retry "overwrites" the good
  result. Neither is shown by the evidence: the tool-calling model generated NO calls, and
  `_merge_corrected_results` (fastapi_server_complete.py:5197) only replaces when
  `corrected_result["corrected"]` is True — a failed retry should not overwrite anything.
  **The unexplained step is how a CORRECT transcription at 12:33:45 became "did NOT
  transcribe any text" at 12:33:57.** Retracted rather than left standing: a cause that fits
  is not a cause that is verified, and every later decision would inherit it.
- **What IS established:** (a) the correct transcription existed; (b) `image_exists` is reset
  to False immediately after forced processing; (c) the retry runs with an empty image and
  cannot succeed; (d) the user is told it failed.
- **NOT fixed — deliberately.** Writing a fix against an unverified cause is the failure this
  log exists to prevent. Next diagnostic step: capture what is actually STORED as the task-1
  result after forced processing, and compare it byte-for-byte with what the arbitrator reads
  at 12:33:57. That single observation should settle it.
- **Clear only when:** a vision request that transcribes correctly REPORTS that transcription,
  asserted by a test that fails on current code.
